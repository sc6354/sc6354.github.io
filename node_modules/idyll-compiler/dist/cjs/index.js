...
var parseFrontMatter = require('gray-matter');
...
var parseFrontMatter__default = /*#__PURE__*/_interopDefaultLegacy(parseFrontMatter);
var _defineProperty__default = /*#__PURE__*/_interopDefaultLegacy(_defineProperty);
var smartquotes__default = /*#__PURE__*/_interopDefaultLegacy(smartquotes);
...
var lex = function lex(options) {
  ...
  var findAliases = function findAliases(name) {
    ...
    return [name].concat(_toConsumableArray__default["default"](aliasNames.filter(function (aliasName) {
  ...
  lexer.addRule(new RegExp(String.raw(_templateObject || (_templateObject = _taggedTemplateLiteral__default["default"](["[s*(", ")s*([^/]]*)s*][\ns\t]*(((?!([s*/(", ")s*])).\n?)*)[\ns\t]*[s*/s*(", ")s*]"], ["\\[\\s*(", ")\\s*([^\\/\\]]*)\\s*\\][\\n\\s\\t]*(((?!(\\[\\s*\\/(", ")\\s*\\])).\\n?)*)[\\n\\s\\t]*\\[\\s*\\/\\s*(", ")\\s*\\]"])), equationAliases, equationAliases, equationAliases), 'i'), function (lexeme, tagName, props, innerText) {
    inComponent = false;
  ...
  lexer.addRule(new RegExp(String.raw(_templateObject2 || (_templateObject2 = _taggedTemplateLiteral__default["default"](["[s*(", ")s*([^/]]*)s*][\ns\t]*(((?!([s*/(", ")s*])).\n?)*)[\ns\t]*[s*/s*(", ")s*]"], ["\\[\\s*(", ")\\s*([^\\/\\]]*)\\s*\\][\\n\\s\\t]*(((?!(\\[\\s*\\/(", ")\\s*\\])).\\n?)*)[\\n\\s\\t]*\\[\\s*\\/\\s*(", ")\\s*\\]"])), codeAlias, codeAlias, codeAlias), 'i'), function (lexeme, tagName, props, innerText) {
    inComponent = false;
    if (this.reject) return;
    updatePosition(lexeme);
...
function pipeline() {
  ...
  return /*#__PURE__*/function () {
    var _ref = _asyncToGenerator__default["default"]( /*#__PURE__*/_regeneratorRuntime__default["default"].mark(function _callee(input) {
      var result,
          _len2,
          args,
          _key2,
...
var _rank;
var rank = (_rank = {}, _defineProperty__default["default"](_rank, idyllAst.VAR, 0), _defineProperty__default["default"](_rank, idyllAst.DERIVED, 1), _defineProperty__default["default"](_rank, idyllAst.DATA, 2), _rank);

function hoistVariables(ast) {
  var variableNodes = idyllAst.queryNodes(ast, idyllAst.isVariableNode);
  variableNodes.sort(function (a, b) {
    return rank[a.type] - rank[b.type];
  });
  idyllAst.removeNodes(ast, idyllAst.isVariableNode);
  idyllAst.prependChildren(ast, variableNodes);
  return ast;
}
...
function flattenChildren(node) {
  if (idyllAst.hasChildren(node)) {
    var flat = [];

    var _iterator = _createForOfIteratorHelper$4(idyllAst.getChildren(node)),
        _step;
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        if (idyllAst.getNodeName(child) === IDYLL_CONTAINER) {
...
function makeFullWidth(ast) {
  ...
  var _iterator = _createForOfIteratorHelper$3(idyllAst.getChildren(ast)),
      _step;
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      try {
        var ist = idyllAst.isTextNode(child);
      } catch (err) {
        console.error(err);
        console.error('AST', JSON.stringify(ast, 0, 2));
      }

      if (idyllAst.isTextNode(child)) {
        textNodes.push(child);
        continue;
      }

      var childName = idyllAst.getNodeName(child).toLowerCase();

      if ([idyllAst.DERIVED, idyllAst.VAR, idyllAst.DATA, FULLWIDTH, SCROLLER].includes(childName) || idyllAst.hasProperty(child, 'fullWidth')) {
        if (childName === FULLWIDTH) {
          child.name = 'div';
          var className = idyllAst.getProperty(child, CLASS_NAME);

          if (className) {
            switch (className.type) {
              case idyllAst.VALUE:
                idyllAst.setValueProperty(child, CLASS_NAME, 'fullWidth ' + className.value);
                break;

              case idyllAst.EXPRESSION:
              case idyllAst.VARIABLE:
                idyllAst.setExpressionProperty(child, CLASS_NAME, "\"fullWidth \" + (".concat(className.value, ")"));
                break;

              default:
                idyllAst.setValueProperty(child, CLASS_NAME, 'fullWidth');
            }
          } else {
            idyllAst.setValueProperty(child, CLASS_NAME, 'fullWidth');
          }
        } else {
          idyllAst.removeProperty(child, 'fullWidth');
        }

        if (textNodes.length) {
          reduced.push(idyllAst.createComponentNode(TEXT_CONTAINER$1, null, textNodes));
          textNodes = [];
        }
...
function wrapText(ast) {
  var query = function query(node) {
    return idyllAst.getNodeName(node) === TEXT_CONTAINER;
  };

  var _iterator = _createForOfIteratorHelper$2(idyllAst.queryNodes(ast, query)),
      _step;

  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      var node = _step.value;
      idyllAst.mapChildren(node, function (child) {
        return idyllAst.isTextNode(child) ? idyllAst.createComponentNode('p', null, child) : child;
      });
    }
  } catch (err) {
    _iterator.e(err);
...
function cleanResults(ast) {
  if (idyllAst.isTextNode(ast)) {
    return ast;
  }

  return idyllAst.mapChildren(ast, function (node) {
    if (idyllAst.isTextNode(node)) {
      return node;
    }

    flattenChildren(node);
    var name = idyllAst.getNodeName(node).toLowerCase();
    var children = idyllAst.getChildren(node); // if not a section, step, or textcontainer component &&
    // node has one child &&
    ...
    if (!['section', 'step', 'textcontainer'].includes(name) && children.length === 1 && idyllAst.getNodeName(children[0]) === 'p' && idyllAst.getChildren(children[0]).length) {
...
function smartQuoteHelper(node) {
  if (idyllAst.isTextNode(node)) {
    node.value = smartquotes__default["default"](node.value);
  } else {
    var name = idyllAst.getNodeName(node).toLowerCase();
    ...
    var _iterator = _createForOfIteratorHelper$1(idyllAst.getChildren(node)),
        _step;
...
function autoLinkify(node) {
  return idyllAst.isTextNode(node) ? linkifyNode(node) : idyllAst.mapChildren(node, autoLinkify);
}
...
function linkifyNode(node) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      var _match = _slicedToArray__default["default"](match, 1),
          href = _match[0];

      var index = match.index; // extract prior text to new text node

      if (index > textIndex) {
        nodes.push(idyllAst.createTextNode(text.slice(textIndex, index)));
      } // generate hyperlink


      nodes.push(idyllAst.createComponentNode('a', {
        href: {
          type: idyllAst.VALUE,
          value: href
        }
      }, [idyllAst.createTextNode(href)])); // update index into source text
  ...
  if (textIndex < text.length) {
    nodes.push(idyllAst.createTextNode(text.slice(textIndex)));
  }

  return idyllAst.createComponentNode('span', null, nodes);
}
...
function _ref() {
  _ref = _asyncToGenerator__default["default"]( /*#__PURE__*/_regeneratorRuntime__default["default"].mark(function _callee(input) {
    var context,
        _parseFrontMatter,
        content,
        data,
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _parseFrontMatter = parseFrontMatter__default["default"](input), content = _parseFrontMatter.content, data = _parseFrontMatter.data;
          ...
          case 15:
            ...
            ast = idyllAst.convertV1ToV2(ast);
            _context.next = 24;
            break;
